...
function handler_sqsConsumerToRedisearchJs() {
    const sourceCodeRepoName = "sqsConsumerToRedisearchJs";
    const objectKey = `config/${sourceCodeRepoName}/${EnvCfgFilename}`;
    const objectBucketName = getValue(exportedValues.Exports, "S3ObjectStoreBucketName");
    const sqsQueueUrl = getValue(exportedValues.Exports, "SqsQueueUrl");
    ...
    for (var i=0; i < args.length; i++) {
        ...
        switch (arg) {
            case "mockIotGpsDeviceAwsSdkV2":
                ...
                break;
            case "sqsConsumerToRedisearchJs":
                handler_sqsConsumerToRedisearchJs();
                break;
            case "reactFrontend":
