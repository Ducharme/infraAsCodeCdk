...
const dateTimeRegex = new RegExp("^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}[:][0-9]{2}:[0-9]{2}\.[0-9]{3}[Z]Z");
...
async function listBucket(config, bucketName, prefix) {
  ...
  const response = await client.send(command);
  const sc = response.$metadata.httpStatusCode;
    if (sc >= 200 && sc < 300 && response.KeyCount > 0) {
      const files = response.Contents.map(item => item.Key);
      console.log(`${response.KeyCount} file(s) found matching ${JSON.stringify(input)}: ${files.join(',')}`);
      return files;
    } else {
      console.log(`No file found matching ${JSON.stringify(input)}`);
      return [];
    }
}
...
function validateDate(fieldName, fieldValue) {
  // 2001-01-01T00:01:02.123Z
  if (fieldValue === undefined || (fieldValue.length > 0 && fieldValue.length != 24))
    throw `Invalid ` + fieldName;
